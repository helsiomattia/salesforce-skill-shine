import type { CompetencyCategory, Skill } from "@/data/competencies";
import { getLocalizedString } from "@/utils/i18nHelper";
import type { jsPDF as JsPDF } from "jspdf";
import type { FinishPanelFormData } from "./finishPanelSchema";

let autoTable: typeof import("jspdf-autotable").default;

type GeneratePdfInput = {
  data: FinishPanelFormData;
...
const addParticipantInfo = (doc: JsPDF, data: FinishPanelFormData, trail: string, lang: SupportedLang, y: number) => {
  const copy = pdfCopy[lang];
...
export const generatePdf = async ({ data, category, ratings, language }: GeneratePdfInput) => {
